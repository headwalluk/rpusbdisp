...
```bash
sudo cp -r . /usr/src/rpusbdisp-1.0.1/
```

Register, build, and install the module:

```bash
sudo dkms add rpusbdisp/1.0.1
sudo dkms build rpusbdisp/1.0.1
sudo dkms install rpusbdisp/1.0.1
```
...
```bash
sudo dkms remove rpusbdisp/1.0.1 --all
```
